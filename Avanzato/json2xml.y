...
elemento: GA tag_name attr_cont_star GC {
		Nodo n = new Nodo();
		n.nome = $2;
		n.attributi = ((Cons)$3).attributi;
		n.figli = ((Cons) $3).figli;
		$$ = n;
	};

tag_name: TAG PP QUOTED {
		$$ = $3.substring(1, $3.length()-1);
		//System.out.println("--------------> "+$$);
	};

attr: AN PP QUOTED {
		Attr a = new Attr();
		a.nome=$1.substring(2,$1.length()-1);
		a.valore = $3;
		$$ = a;
	}; 
attr_cont_star: V attr_cont  attr_cont_star {
		((Cons)$3).push($2);
		$$ = $3;
	}
	| /*vuoto*/ {
...
content: CONTENT PP QA p_figlio QC {
		$$ = $4;
	};
...
p_figlio: figlio figlio_star {
		((ArrayList<Object>)$2).add(0,$1);
		$$ = $2;
	}
	| /*vuoto*/ {
...
figlio_star: V figlio figlio_star {
		((ArrayList<Object>)$3).add(0,$2);
		$$ = $3;
	}
	| /*vuoto*/ {
		$$ = new ArrayList<Object>();
...
public static void main(String args[]) throws Exception {
	...
	if ( args.length > 0 ) {
		...
		System.out.println(yyparser.radice.toXML(0));
	} else {
